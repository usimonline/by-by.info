...
if(empty($_POST['post_1'])) {
    ...
    for ($i = 0; ; $i++) {
        ...
        if ($all_count == 0) {
            ...
            $EndWord = "</description>";
            $content_description[$k] = otbor_parse($content, $StartWord, $EndWord);//получили text, хотя называетя description, просто не хочу переделывать
...
//echo $total_parse_all;
    ...
    require_once("parser_insert_news_vz.php");
}
